...
def build_merge_messages(chunk_outputs):
    text = "\n\n".join(
        [f"[Chunk {c['chunk_id']}]\n{c['output']}" for c in chunk_outputs[:15]]
    )
...
def extract_original_title(record):
    ...
    if isinstance(content, list) and content:
        title = content[0].get("Literature Title", "")
...
def main():
    ...
    lora_path = "/root/autodl-tmp/NLP_Project/outputs/qwen25_7b_lora_fold3_v3_reduced"
    ...
    with open(args.output_file, "a", encoding="utf-8") as out:
        for _, record in records:
            original_title = extract_original_title(record)
            messages = record.get("messages", [])
            if len(messages) < 2:
                continue
            ...
            predictions = parse_predictions(merged)
