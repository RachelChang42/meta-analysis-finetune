cd /root/autodl-tmp/NLP_Project

cat > run_fold_4_5_full.sh <<'SH'
#!/bin/bash
set -e
...
# 确保推理聚合使用前15个chunk
sed -i 's|chunk_outputs\[:[0-9]\+\]|chunk_outputs[:15]|g' scripts/extract_and_aggregate_lora_v2.py

for FOLD in ${FOLDS}
do
  echo "========== Fold ${FOLD}: build chunk train =========="
  ...
  echo "========== Fold ${FOLD}: patch train script =========="
...
PY
  ...
  python ./eval/eval_script_finaluse.py \
    ...
    > outputs/eval_lora_fold${FOLD}_v3_top15.txt

  cat outputs/eval_lora_fold${FOLD}_v3_top15.txt
