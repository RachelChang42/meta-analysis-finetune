...
def parse_json_line_with_flag(raw):
    ...
    try:
        ...
        start = min(start_candidates) if start_candidates else -1  # Ensure consistent start index detection
...
def parse_json_stream(text, warnings=None, source=""):
    ...
    while idx < length:
        ...
        if idx >= length:
            break
        try:
            obj, end = decoder.raw_decode(text, idx)  # Ensure proper closing brackets
            objs.append(obj)
            idx = end
        except json.JSONDecodeError:
            next_positions = [text.find("{", idx + 1), text.find("[", idx + 1)]
            next_positions = [p for p in next_positions if p != -1]
            if not next_positions:  # Check for valid next positions
                break
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("用法: python eval_script_finaluse.py <模型输出.jsonl> <标注数据.jsonl> [--no-normalize]")
    else:
